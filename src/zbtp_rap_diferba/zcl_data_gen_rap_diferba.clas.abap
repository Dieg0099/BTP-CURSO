CLASS zcl_data_gen_rap_diferba DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_DATA_GEN_RAP_DIFERBA IMPLEMENTATION.
  ...
  out->write( 'Adding Travel data' ).
    ...
    DELETE FROM ztravel_dfbp_a.

    INSERT ztravel_dfbp_a FROM (
    ...
    DELETE FROM zbooking_dfbp_a.

    INSERT zbooking_dfbp_a FROM (

        SELECT
          FROM /dmo/booking
          JOIN ztravel_dfbp_a ON /dmo/booking~travel_id = ztravel_dfbp_a~travel_id
          JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
          FIELDS  "client,
                  uuid( ) AS booking_uuid,
                  ztravel_dfbp_a~travel_uuid AS parent_uuid,
                  ...
                  ztravel_dfbp_a~last_changed_at AS local_last_changed_at ).


    DELETE FROM zbksppl_dfbp_a.

    out->write( 'Adding Booking Supplements data' ).

    INSERT zbksppl_dfbp_a FROM (
       SELECT FROM /dmo/book_suppl AS supp
              JOIN ztravel_dfbp_a  AS trvl ON trvl~travel_id = supp~travel_id
              JOIN zbooking_dfbp_a AS book ON book~parent_uuid = trvl~travel_uuid
